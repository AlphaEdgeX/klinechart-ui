...
export const fibonacciSpeedResistanceFan: OverlayTemplate = {
  ...
  createPointFigures: ({ coordinates, bounding }) => {
    ...
    if (coordinates.length > 1) {
      ...
      percents.forEach((percent) => {
        ...
        const ray1 = getRayLine(
          [coordinates[0], { x, y: coordinates[1].y }],
          bounding
        );
        const ray2 = getRayLine(
          [coordinates[0], { x: coordinates[1].x, y }],
          bounding
        );
        lines2 = lines2.concat(ray1 as LineAttrs[]);
        lines2 = lines2.concat(ray2 as LineAttrs[]);
        texts.unshift({
